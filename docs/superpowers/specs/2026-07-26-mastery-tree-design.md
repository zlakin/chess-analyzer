...
- Levels below the active one (mastered, superseded) draw only from whatever backfill/mistake cards were already SM-2-tracked for that specific node — no new user-mistake inflow, per the rule above.
