...
- **Can't solve** (only enabled once `hintUsed`): reveals `bestMoveUci` as an arrow against `fenBefore` (existing reveal mechanism — unchanged), shows the answer, enables **Next**. Records the card's outcome as `gaveUp` (see §2). **Revised post-implementation (final whole-branch review caught this as a Critical bug):** if no SRS review has been submitted for this card yet (i.e. the user hinted and gave up without ever attempting a move), Can't solve *also* submits `submitPuzzleReview(cardId, 0)` — the original "does not touch SRS" rule left a card with no attempts permanently `due` forever, since `buildPuzzleQueue` synthesizes a fresh due-now state for any card with no stored SRS entry. Quality 0 is SM-2's correct grade for "couldn't recall it" and preserves first-resolution-wins (a card that already has a submitted review from an earlier wrong attempt is untouched, per the existing `reviewSubmitted`/`claimReview` guard).
