...
This is a pure data-layer change with no UI dependency — a `phase:middlegame` finding at `bullet` scope (69%, higher significance because bullet's smaller sample size and higher share combine to a bigger score) naturally wins over the `overall` scope (64%) or `rapid` scope (61%) version automatically, with no special-casing of which bucket "should" win.
